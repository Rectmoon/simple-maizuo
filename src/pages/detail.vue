    ...
    <div class="desc">
      ...
      <div class='info'>
        <p>导&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;演：
          {{detail.director}}</p>
        <p>主&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;演
          <span v-for='(item,i) in detail.actors'>{{item.name}}</span>
        </p>
      ...
      <router-link 
        :to="{name:'cinema',params:{id:detail.id}}"
        v-if="detail.isNowPlaying"
        class='go-pay'>
        立即购票
      </router-link>
    </div>
...
<script>
  ...
  export default {
    methods: {
      ...mapActions(['getFilmInfo'])
    },
    computed: {
      ...mapGetters({detail:'getFilmInfo'})
    },
    filters: {
      getDay(time) {
